...
同一个服务进程内，订单文件写入会排队执行，避免两个买家同时下单时互相覆盖订单记录。
...
服务会校验支付宝网关返回签名，也会核对订单号、应用 ID 和付款金额。预下单、查单或异步通知签名无效，或订单号、应用 ID、付款金额不一致时，不会生成授权码。

如果自动邮件第一次发送失败，订单状态会返回 `emailDeliveryStatus=failed` 和 `emailDeliveryError`。后续再次查询订单会重试发送；授权码本身不重新生成。
