...
export async function buildAutoRevenueStatus(options = {}) {
  ...
  const steps = [
    ...
    step(9, "准备支付宝商家参数", alipayMerchantReady, alipayMerchantReady ? "已发现可通过预检的支付宝商家参数。" : alipayEnvEvidence(alipayEnvFile), "查看 launch/alipay-launch-readiness.md，补 ALIPAY_APP_ID、ALIPAY_PRIVATE_KEY、ALIPAY_PUBLIC_KEY、OFFERDESK_PUBLIC_BASE_URL。", "blocked"),
    ...
    step(13, "写真实环境变量文件", alipayFullEnvReady, alipayFullEnvReady ? "当前运行环境或本地 env 文件已通过支付宝预检。" : alipayEnvEvidence(alipayEnvFile), "填完 env 后运行 node scripts/alipay-launch-readiness.mjs --write launch/alipay-launch-readiness.md --no-fail。", "blocked"),
